// raw-weapons/ 폴더의 원본 무기 아이콘들을 모두 정사각형(256x256)으로 리사이즈해서
// public/weapons/ 에 WebP로 저장합니다(scripts/prepare-character-images.mjs와 동일한 방식).
//
// 사용법:
//   1. npm install --save-dev sharp   (최초 1회, 이미 설치돼 있으면 생략)
//   2. raw-weapons/ 폴더에 원본 아이콘 넣기 (파일명이 곧 무기 id — config/weapons.js의 WEAPONS 키)
//   3. npm run prepare-weapons
//
...
const SRC_DIR = fileURLToPath(new URL('../raw-weapons/', import.meta.url))
const OUT_DIR = fileURLToPath(new URL('../public/weapons/', import.meta.url))
...
async function main() {
  ...
  if (files.length === 0) {
    console.log('raw-weapons/ 에 이미지가 없습니다. 원본 아이콘을 먼저 넣어주세요.')
    return
  }
